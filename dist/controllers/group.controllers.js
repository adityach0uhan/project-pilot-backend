...
export const getAllGroups = async (req, res, next) => {
    try {
        ...
        const groups = await GroupModel.find({ semester }).select('name groupNumber project inviteCode semester createdAt updatedAt');
        if (!groups) {
...
export const updateGroup = async (req, res, next) => {
    try {
        ...
        const group = await GroupModel.findById({ _id: groupId }, { new: true });
        if (!group) {
            res.status(404).json({ message: 'Group not found' });
            return;
        }
